...
export default function Nav() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Progress from 0 to 1 over the first 24px of scroll
      const progress = Math.min(1, scrollY / 24)
      setScrollProgress(progress)
      // Nav becomes fixed when it reaches the header button level (top: 16px)
      // Nav starts at: 24px padding + 98px logo + 16px margin = 138px
      // Fixed at top: 16px, so fix when scrollY >= 138 - 16 = 122px
      setNavFixed(scrollY >= 122)
    }
  ...
  return (
    <>
      {/* ── Utility Bar (desktop) ── */}
      <div
        className="hidden md:flex items-center justify-between px-6 text-xs fixed top-0 left-0 right-0 z-50"
        style={{
          height: 77,
          backgroundColor: navFixed ? 'rgba(0, 0, 0, 0.5)' : 'transparent',
      ...
      >
        ...
        <div className="flex items-center">
          {pathname === '/' ? (
            <button
              ...
              className="absolute text-black hover:text-ralph-pink"
          ...
          {!user && (
            <>
              <Link
                href="/join-ralph"
                className={`border-2 border-white text-white px-4 flex items-center justify-center ${
              ...
              <Link
                href="/login"
                className="border-2 border-white text-white px-4 ml-6 transition-colors flex items-center justify-center bg-transparent hover:bg-ralph-pink"
                style={{
                  borderRadius: 22,
                  height: 44,
                  fontFamily: "'Gooper Trial', serif",
        ...
        <div className="flex items-center gap-6">
          <Link
            href="/work-with-us"
            className={`border-2 border-white text-white px-4 transition-colors flex items-center justify-center ${
      ...
      <nav className="relative z-[60]">
        {/* Desktop */}
        <div className="hidden md:flex flex-col items-center" style={{ paddingTop: 24, paddingBottom: 32 }}>
          {/* Logo */}
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`relative text-btn text-[22px] transition-colors ${
                    isActive
                      ? 'text-primary'
